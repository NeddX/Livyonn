    ...
    void MoveHandler(Runtime& r)
    {
		...
		if (r.pc->reg1.ptr)
		{
			...
			switch (r.pc->pl)
			{
				...
				default:
				{
					r.stack.WriteAt64(r.regs[r.pc->reg1.reg] - 1, rhv);
                    break;
				}
			}
		}
		else
		{
            if (r.pc->reg2.ptr)
            {
                if (r.pc->reg1.reg != NUL) rhv = r.regs[r.pc->reg1.reg];
                else rhv = r.pc->p3;
                ...
                switch (r.pc->pl)
                {
                    ...
                    default:
                    {
                        r.stack.WriteAt64(r.regs[r.pc->reg2.reg] - 1, rhv);
                        break;
                    }
                }
            }
...
#define s_buffer r.stack.buffer // Makes things easier
    ...
    {
        Instruction* addr = (Instruction*) (r.stack.ReadFrom64(r.regs[RBP] - 4));
